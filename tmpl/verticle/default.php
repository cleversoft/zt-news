...
<div class="zt_news_wrap">
    <div class="zt-frame-cat">
        <?php foreach ($groupItems as $items): ?>
            <div class="zt-category" style="width: <?php echo $width; ?>%">
                <!--Title Block-->
                <?php if ($showTitleCategory): ?>
                    <div class="title_cat clearfix">
                        <h2 class="title">
                            <a href="<?php echo $items['category']->link; ?>"
                               alt="<?php echo $items['category']->title; ?>"><?php echo $items['category']->title; ?></a>
                        </h2>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="row-fluid">
                <div class="news_lead">
                    <?php
                    $numberIntroItems = (count($items['items']) <= $numberIntroItems) ? count($items['items']) : $numberIntroItems;
                    foreach ($items['items'] as $item) :
                        ?>
                        <div class="zt-article-item">
                            <?php if (@$item->subThumb != '' && $params->get('is_image', 1) == 1): ?>

                                <a href="<?php echo $item->link; ?>" title="<?php echo $item->title; ?>">
                                    <img class="thumbnail" src="<?php echo $item->subThumb; ?>"
                    ...
                    <?php endforeach; ?>
                    <?php if ($numberIntroItems < count($items['items'])): ?>
                        <div class="article-item">
                            <?php foreach ($items['items'] as $item) : ?>
                                <div class="more_item <?php
                                if ($j == $numberIntroItems):
                                    echo 'first-item';
                                elseif ($j == (count($items['items']) - 1)):
                                    echo 'last-item';
                                endif;
                                ?>">
